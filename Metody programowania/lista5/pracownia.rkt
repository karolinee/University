...
(define (remove x xs)
  (define (aux xs acc)
    (if (null? xs)
        ...
        (if (eq? x (car xs))
            (append acc (cdr xs))
            (aux (cdr xs) (append acc (car xs))))))
  (aux xs null))

(define (merge-clause c1 c2)
  (clause (append (clause-lits c1) (clause-lits c2))))

(define (proof-result pf)
  (if (res? pf)
      (merge-clause (remove (pos (res-var pf)) (proof-result (res-proof-pos pf))) (remove (neg (res-var pf)) (proof-result (res-proof-neg pf))))
      (axiom-clause pf)))
